...
4. Dla wektorów na płaszczyźnie: $\mathbf{a}=[3,4]$ i $\mathbf{b}=[-4,3]$ oblicz ich iloczyn skalarny i sprawdź, czy są prostopadłe. Wyznacz rzut wektora $\mathbf{a}$ na $\mathbf{b}$.

5. Oblicz długość wektora $\mathbf{c} = [1, 1]$ i znajdź wersor tego wektora.

6. Oblicz długość wektora $\mathbf{c} = [1, 2, 3]$ i znajdź wersor tego wektora.

7. Oblicz pole trójkąta rozpiętego na wektorach $[2, 1, 2]$ i $[-1, 1,1]$.

8. Oblicz kąt w stopniach między wektorami $[4,2,1]$ i $[1,3,2]$.

9. Znajdź współrzędne środka odcinka o końcach $A(-1, 2)$ i $B(3, -2)$.
   
10. Dla trójwymiarowych wektorów: $\mathbf{a}=[a_x, a_y, a_z]$, $\mathbf{b}=[b_x, b_y, b_z]$, $\mathbf{c}=[c_x, c_y, c_z]$, udowodnij, że spełniona jest następująca tożsamość:
...
13.  Dla jakich wartości parametru $s$ wektory $\mathbf{p}=[s,2,1-s]$ i $\mathbf{q}=[s,1,-2]$ są prostopadłe?
